<?
	...
	if (!$text)
	{
		...
		{
			$result=mysql_query("SELECT id FROM mhlp_volumes ORDER BY id DESC LIMIT 1;");
			$res_object=mysql_fetch_object($result);
...
<div class="header3" style="float:left"><?=$misc_text[2][$sess->lang]?></div><div style="float:right; cursor:pointer;" onclick="hide_toc();"><b><?=$misc_text[3][$sess->lang]?> [x]</b></div><br clear="all" /><br />
...
<div style="padding-bottom:5px;">
	<div style="font-size:8pt;"><a href="page.php?id=browser_mjlh3&text=<?=$row->id?>"><?=$row->title?></a></div>
</div>
<?
}
...
<table width="876" border="0">
  ...
  <tr>
    ...
    <td width="475" align="center"><div class="header2"><?=$misc_text['vol3'][$sess->lang]?></div>
...
<?
$curpage = $curtext->page_low;
?>
<img id="currentPage" name="currentPage" src="texts/volume<?=$curtext->volume?>/png/MJLH-full-text-<?=$curpage?>.png" style="border: 1px solid #CCCCCC; border-bottom:1px solid #EEEEEE; border-top:1px solid #EEEEEE;" width="876"; height="1239"; />
...
function goToPage(page,jump)
{
	...
	z_page = parseFloat(page);
	document.getElementById('currentPage').src="texts/volume<?=$curtext->volume?>/png/MJLH-full-text-" + z_page + ".png";
